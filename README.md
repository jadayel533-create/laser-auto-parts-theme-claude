...
Custom RTL-first storefront theme for [Laser Auto Parts](https://laserautoparts.com), built on [Zid's Vitrin platform](https://zid.sa). Forked from Zid's official [Growth Theme](https://github.com/zidsa/growth-theme) reference template and rebranded for Laser Auto Parts.

**TailwindCSS v4** | **Vite 7** | **Jinja2** | **RTL-first (Arabic)**

## Status

Development scaffold. No live-store activation or publishing has been performed from this repository. Pushing, previewing, and activating on the Zid store must be done from a local machine (or CI) authenticated via `vitrin login` (Zid OAuth) — this repo only holds the theme source.

## Intended workflow

```
GitHub (this repo) → Vitrin CLI (build / validate / preview) → Zid store
```

1. Edit theme files here and push to GitHub as normal.
2. Locally: `git clone` this repo, `npm install`, `npm run build`.
3. `vitrin login` once (opens a browser against your Zid partner account).
4. `vitrin preview <dev-store-id> --build` to see it on a dev store.
5. Only once approved: `vitrin push --store <id> --activate` to go live.
